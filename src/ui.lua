...
G.FUNCS.berry_credits_sprite = function(self)
	...
	-- print(target)
	self.UIBox:get_UIE_by_ID('berry_center_sprite').config.object.sprite_pos.x = 0
	-- return ret
end

G.FUNCS.tony_hover = function(self)
	-- if G.CONTROLLER.hovering.prev_target ~= G.CONTROLLER.hovering.target then
	if G.CONTROLLER.focused and G.CONTROLLER.focused.target == self.target then
		-- os.execute("cls")
		-- print(inspectDepth(G.CONTROLLER.focused, 0, 3))
	end
	-- print(self.states.hover.is)
	if self.states.hover.is then
		target = self.config.target or 1
		-- print('meow', target)
	end
	-- return dp.hovered
end
-- Credits!
BerryLegendaries.mod.extra_tabs = function()
	local target = 0
    return {
        {
            label = 'Credits',
